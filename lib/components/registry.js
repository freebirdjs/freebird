...
function registerGadget(freebird, gad, callback) {
    ...
    if (gad._recovering) {   // it was registered before, and now this instance is recovered from database
        freebird._gadbox.set(gadId, gad, function (err, id) {
            if (!err)
                gad._recovering = false;
        ...
        freebird._gadbox.add(gad, function (err, id) {
            if (!err) {
                ...
                var oldRec = _.find(gadTbl, function (gadRec) {
                    return gadRec.auxId === gad.get('auxId');
                });

                if (oldRec) {
                    oldRec.gadId = id;
                } else {
                    // push new gadget record to gadTable
                    gadTbl.push({ gadId: id, auxId: gad.get('auxId') });
                }

                freebird._devbox.replace(devId, 'gads', gadTbl, function (er) {
                    callback(er, id);
                });
            } else {
                callback(err, id);
