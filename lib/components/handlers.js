...
handlers.ncDevIncoming = function (msg) {   // { netcore: nc, permAddr: permAddr, raw: rawDev }
    ...
    if (!_.isFunction(nc.cookRawDev))       // no cooker, do nothing
        return freebird._tweet(EVT_TOP.ERROR, new Error('Raw device cooker, cookRawDev(), is not implemeneted on netcore: ' + nc.getName()));   // [TODO] bad signature
    ...
    nc.cookRawDev(devIn, msg.raw, function (err, brewedDev) {
        if (err)
            return freebird._tweet(EVT_TOP.ERROR, err); // [TODO] bad signature
        ...
        if (dev) {  // dev already exists, no need to fire EVT_TOP.DEV_INCOMING
            ...
            freebird.register('device', devIn, function (err, id) {   // joinTime tagged in freebird.register('device')
                if (err) {
                    devIn = null;
                    freebird._tweet(EVT_TOP.ERROR, err, err);         // register fail // [TODO] bad signature
                } else {
                    devIn.enable();
                    devIn._poke();
                    devIn.set('net', { status: 'online' });

                    freebird._fire(EVT_TOP.DEV_INCOMING, devIn);
...
handlers.ncDevReporting = function (msg) {      // { netcore: nc, permAddr: permAddr, data: devAttrs }
    ...
    fb._tweet(EVT_TOP.DEV_REPORTING, msg);  // [TODO] bad signature
};  // { netcore: nc, devId: id, permAddr: permAddr, data: devAttrs }

// v [TODO] should check classId, if props.name === 'unknown' => let props.name = classId
handlers.ncGadIncoming = function (msg) {  // { netcore: nc, permAddr: permAddr, auxId: auxId, raw: rawGad }
    var fb = this,
        nc = msg.netcore,
        ncName = nc.getName(),
        dev = fb.findByNet('device', ncName, msg.permAddr),
        gad = fb.findByNet('gadget', ncName, msg.permAddr, msg.auxId),
        gadIn,
        fbMsg = { gad: null },
        wsMsg = _makeWsMsg(msg, { id: null, data: null }),  // [TODO] _makeWsMsg, wsMsg
        doGadCook,
        syncTimes = 0;
    ...
    if (!_.isFunction(nc.cookRawGad)) {
        ...
        fb._tweet(EVT_TOP.ERROR, noCookerError, noCookerError);     // [TODO] bad signature
        ...
        nc.cookRawGad(gadIn, msg.raw, function (err, brewedGad) {
            ...
            if (gad) {
                ...
                fb.register('gadget', gadIn, function (err, id) {
                    if (err) {
                        gadIn = null;
                        fb._tweet(EVT_TOP.ERROR, err, err);      // register fail // [TODO] bad signature
                    } else {
                        gadIn.enable();
                        fbMsg.gad = gadIn;
                        wsMsg.id = gadIn.get('id');
                        wsMsg.data = utils.dumpGadgetInfo(gadIn);
                        fb._tweet(EVT_TOP.GAD_INCOMING, fbMsg, wsMsg);  // [TODO] bad signature
                    }
                });
            }
        });
...
handlers.ncGadLeaving = function (msg) {   // { gad: gad }
    ...
    fb.unregister('gadget', msg.gad, function (err) {
        if (err)
            fb._tweet(EVT_TOP.ERROR, err, err); // if fails, gad won't be removed // [TODO] bad signature
        else
            fb._tweet(EVT_TOP.GAD_LEAVING, fbMsg, wsMsg);   // [TODO] bad signature
    });
};
...
handlers.ncGadReporting = function (msg) { // { netcore: nc, permAddr: permAddr, auxId: auxId, data: gadAttrs }
    ...
    fb._tweet(EVT_TOP.GAD_REPORTING, fbMsg, wsMsg); // [TODO] bad signature
};
...
handlers.devError = function (msg) {       // { error: err }: err, err.info = { netcore: ncName, dev: id }
    // console.log('#########################');
    // console.log(msg);
    this._tweet(EVT_TOP.ERROR, msg, msg); // [TODO] bad signature
};
...
function bannedComponent(fb, type, indType, msg, cb) {
    ...
    fb._tweet(evtName, upMsg);  // [TODO] // [TODO] bad signature
...
function updateComponent(fb, type, namespace, msg, cb) {
    ...
    box.modify(id, namespace, delta, function (err, diff) {
        ...
        fb._tweet(evtName, fbMsg, wsMsg);      // instance always been changed // [TODO] bad signature

        if (evtName === EVT_TOP.DEV_NET_CHANGED && delta.status) {
             fb._tweet(EVT_TOP.DEV_STATUS_CHANGED, {    // [TODO] bad signature
                    dev: fbMsg.dev,
...
module.exports = function attachEventListeners(freebird) {
    ...
    // [TODO] No ncLsns.gadError implemented
