...
handlers.ncDevIncoming = function (msg) {   // { ncName, permAddr, raw: rawDev }
    ...
    function queueEvent(msg) {
        gadEventQueue.push(msg);
    }

    freebird.on(EVT_BTM.NcGadIncoming, queueEvent);

    function releasQueueEvent() {
        if (gadEventQueue.length !== 0) {
            freebird.emit(EVT_BTM.NcGadIncoming, gadEventQueue.shift());
            setImmediate(releasQueueEvent);
        } 
    }
...
function bannedComponent(fb, type, indType, msg, cb) {
    ...
    if (type === 'device') {
        ...
        if (component && component._removing) {
            nc.remove(permAddr, function (err) {
                if (err) {
                    fb._fire('warn', err);
