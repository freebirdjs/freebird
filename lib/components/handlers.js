...
handlers.ncDevIncoming = function (msg) {   // { ncName, permAddr, raw: rawDev }
    ...
    if (!_.isFunction(nc.cookRawDev))       // no cooker, do nothing
        return freebird._tweet('net', 'error', 0, {
            netcore: msg.ncName,
            message: 'Raw device cooker, cookRawDev(), is not implemeneted on netcore: ' + msg.ncName
        });
...
handlers.ncGadIncoming = function (msg) {  // { ncName, permAddr, auxId, raw: rawGad }
    ...
    if (!_.isFunction(nc.cookRawGad)) {
        ...
        freebird._fire('error', noCookerError);
        freebird._tweet('net', 'error', 0, { netcore: ncName, message: noCookerError.message });
...
handlers.gadAttrsAppend = function (msg) {
    ...
    var fb = this,
        ...
        id = msg.id,
        attrs = msg.data,
        fbMsg = { gad: msg.gad, data: attrs },  // [TODO] use msg.id to get gadget instance
