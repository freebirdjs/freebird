...
handlers.ncEnabled = function (msg) {       // { netcore: nc }
    this._fire(EVT_TOP.ENABLED, msg);

    this._tweet('net', 'enabled', 0, {
...
handlers.ncDisabled = function (msg) {      // { netcore: nc }
    this._fire(EVT_TOP.DISABLED, msg);
    this._tweet('net', 'disabled', 0, {
        netcore: msg.netcore.getName()
    });
};
...
handlers.ncStarted = function (msg) {       // { netcore: nc }
    this._fire(EVT_TOP.STARTED, msg);

    this._tweet('net', 'started', 0, {
        netcore: msg.netcore.getName()
    });
};
...
handlers.ncStopped = function (msg) {       // { netcore: nc }
    this._fire(EVT_TOP.STOPPED, msg);
...
handlers.ncPermitJoin = function (msg) {    // { netcore: nc, timeLeft: ticks }
    ...
    this._tweet('net', 'permitJoining', 0, {
        netcore: msg.netcore.getName(),
...
handlers.ncDevIncoming = function (msg) {   // { netcore: nc, permAddr: permAddr, raw: rawDev }
    ...
    if (!validate.isNetcore(nc))
        return fb._tweet(EVT_TOP.ERROR, new Error('Invalid netcore'));
    else if (!_.isFunction(nc.cookRawDev))       // no cooker, do nothing
        ...
        if (dev) {  // dev already exists, no need to fire EVT_TOP.DEV_INCOMING
            ...
            fb.register('device', devIn, function (err, id) {   // joinTime tagged in fb.register('device')
                if (err) {
                    ...
                    fb._tweet(EVT_TOP.DEV_INCOMING, {
                        netcore: nc,
                        permAddr: msg.permAddr,
                        id: devIn.get('id'),    // [TODO] dev, devId, or id?
                        data: utils.dumpDeviceInfo(devIn)
                    });
                }
            });
        } else {
...
handlers.ncDevLeaving = function (msg) {   // { netcore: nc, permAddr: permAddr }
    ...
    if (dev._removing) {                            // manually remove, should unregister all things
        dev.get('gadTable').forEach(function (rec) {
            var gad = fb.findById('gadget', rec.gadId);
            if (gad)
                gad._fire(EVT_BTM.NcGadLeaving, { netcore: nc, gad: gad }); // Internal Event
        });

        fb.unregister('device', dev, function (err) {
            if (err)
                fb._tweet(EVT_TOP.ERROR, err);
            else
                fb._tweet(EVT_TOP.DEV_LEAVING, { netcore: nc, permAddr: msg.permAddr, id: devId });
        });
    }
};
...
function bannedComponent(fb, type, indType, msg, cb) {
    ...
    if (type === 'dev') {
        component = fb.findByNet('device', ncName, permAddr);
        if (component) {
        ...
        component = fb.findByNet('gadget', ncName, permAddr, msg.auxId);
