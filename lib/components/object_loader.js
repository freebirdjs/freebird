...
loader.reloadSingleGad = function (fb, gadRec, callback) {
    var ncName = gadRec.netcore,
        ...
        hisDev = fb.findDevByAddr(ncName, permAddr),
        gad,
        recoveredGad;
    ...
    gad = fb.findGadByAddrAuxId(ncName, permAddr, gadRec.auxId);
...
loader.sync = function (fb, callback) {
    loader._syncGads(function (err) {
        if (err) {
            callback(err);
        } else {
            loader._syncDevs(function (err) {
                callback(err);
            });
        }
    });
