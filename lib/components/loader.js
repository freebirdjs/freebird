...
loader.reloadSingleDev = function (freebird, devRec, callback) {
    ...
    if (!nc)    // netcore is not registered or dev exists, do not reload
        return utils.feedbackNextTick(null, null, callback);
...
loader.reloadSingleGad = function (freebird, gadRec, callback) {
    var ncName = gadRec.netcore,
        ...
        hisDev = freebird.findFromNetcore(ncName, permAddr),
        gad = freebird.findFromNetcore(ncName, permAddr, gadRec.auxId),
        recoveredGad;

    if (!hisDev || gad)
        return utils.feedbackNextTick(null, null, callback);    // find no device or gad is there,  do not load
...
loader.reloadByType = function (freebird, type, callback) {
    ...
    if (type === 'dev' || type === 'device') {
        ...
        return utils.feedbackNextTick(new Error('Unknown type: ' + type), null, callback);
    }
...
loader.unloadByNetcore = function (freebird, ncName, callback) {
    ...
    if (validate.callback(callback) && !nc)
        return utils.feedbackNextTick(new Error('netcore not found.'), null, callback);
