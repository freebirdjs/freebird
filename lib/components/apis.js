...
//     ],
//     devApiNames = [ 'devRead', 'devWrite', 'devIdentify', 'devEnable', 'devDisable', 'devGetProps', 'devSetProps' ],
//     gadApiNames = [ 'gadRead', 'gadWrite', 'gadExec', 'gadSetReportCfg', 'gadGetReportCfg', 'gadEnable', 'gadDisable', 'gadGetProps', 'gadSetProps' ];

/**********************************************************************************/
/*** APIs for client (e.g. websocket)                                           ***/
/**********************************************************************************/
// ok
apis.getAllDevIds = function (args, callback) {   // { [ncName:String] }
    ...
    return setImmediate(callback, null, { ids: ids });
...
apis.getDevs = function (args, callback) {        // { ids:Number[] }
    ...
    var self = this,
        devs = (args.ids).map(function (id) {
            ...
            return dev ? dumpDeviceInfo(dev) : undefined;   // [TODO] dumpDeviceInfo in utils
        });

    return setImmediate(callback, null, { devs: devs });
};  // return { devs: [ devInfo, ... ] }
...
apis.getGads = function (args, callback) {        // { ids:Number[] }
    ...
    return setImmediate(callback, null, { gads: gads });
};  // return { gads: [ gadInfo , ... ] }
...
apis.getNetcores = function (args, callback) {    // { [ncNames:String[]] }
    ...
    return setImmediate(callback, null, { netcores: ncs });
...
apis.getBlacklist = function (args, callback) {   // { ncName:String }
    ...
    if (!utils.hasCalledWithNotFoundError('netcore', nc, callback))
        return setImmediate(callback, null, { list: nc.getBlacklist() });
...
apis.gadGetProps = function (args, callback) {    // { id:Number, propNames:String[] }
    ...
    if (!utils.hasCalledWithNotFoundError('gad', gad, callback)) {
        try {
            ...
            return setImmediate(callback, null, { props: props });
        } catch (e) {
            return setImmediate(callback, e);
        }
    }
