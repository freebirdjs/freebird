...
Agent.prototype.addTransport = function (name, transp, callback) {
    var self = this,
        nameConflict = false,
    ...
    nameConflict = _.some(this._transports, function (trsp) {
        return trsp._name === name;
    });

    if (nameConflict)
        return setImmediate(callback, new Error('name of transportation conflicts: ' + name));
...
Agent.prototype.incomingMessageHandler = function (transp, msg) {   // msg: { clientId, data }
    // msg.data could be the json strings of { __intf, subsys, seq, id, cmd, args } (REQ)
    ...
    this.parseIncomingMessage(msg, function (result) {              // { valid: true, data: jmsg }
        ...
        if (!result.valid || utils.rpcIntfString(jmsg.__intf) !== 'REQ') {
            ...
            self.incomingRequestHandler(transp, jmsg, function (rsp) {
                transp.send({ clientId: clientId, data: JSON.stringify(rsp) }, function (err) {
                    if (err)
                        self.emit('error', err);
                });
            });
...
Agent.prototype.incomingRequestHandler = function (transp, reqObj, callback) {
    ...
    if (!_.isFunction(api)) {
        ...
        api.call(freebird, args, function (err, result) {
            //  RSP: { __intf, subsys, seq, id, cmd, status, data }
...
Agent.prototype.indicate = function (msg, callback) {
    var freebird = this._freebird,
        numTransp = this._transports.length,
