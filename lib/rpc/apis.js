'use strict';
// [TODO] reset, maintain [TODO] timeout status?
// [TODO] gadSetReportCfg - result FIX spec
...
apis.permitJoin = function (args, callback) {     // args = { [ncName:String], duration:Number }
    ...
    if (typeErr || validate.durationTypeError(args.duration))
    ...
    if (_.isString(args.ncName)) {
        ...
        _.forEach(this._netcores, function (netcore) {
            netcore.permitJoin(args.duration, function (err) {
                if (err) {
...
apis.reset = function (args, callback) {          // args = { ncName:String, mode:Number } => [TODO] mode
    var nc,
        ncNum,
        SOFT_RESET = 0,
        mode = _.isNil(args.mode) ? SOFT_RESET : args.mode,
        typeErr = _.isNil(args.ncName) ? undefined : validate.ncNameTypeError(args.ncName);
