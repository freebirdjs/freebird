...
function Freebird(name, netcores, options) {
    ...
    if (!(this instanceof Freebird))
        return new Freebird(name, netcores, options);
    ...
    var maxDevNum = options.maxDevNum || 200,
        ...
        devDbPath = (_.isObject(options.dbPaths) ? opt.dbPaths.device : undefined) || devboxPath,
    ...
    // [TODO] need ncNames? How to find duplicates?
    var ncNames = [];
...
Freebird.prototype.addTransport = function (name, transp, callback) {
    // [TODO] check name, is string? duplicate?
...
Freebird.prototype._tweet = function (subsys, indType, id, data) {  // Send RPC indications
    var self = this,
        ind = { __intf: 'IND', subsys: subsys, type: 'indType', id: id, data: data };
    ...
    setImmediate(function () {
        self._apiAgent.indicate(ind, function (err) {
            self._fire();   // [TODO] error event name? -> call directly
        });
    });
};
